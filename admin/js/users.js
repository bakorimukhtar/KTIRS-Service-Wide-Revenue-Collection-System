...
function getPrimaryMdaNameForUser(userId) {
  ...
  if (!scope) return '—';
  const mda = allMdas.find((m) => m.id === scope.mda_id);
  return mda ? mda.name : '—';
}
...
(async () => {
  ...
  // 2) Load MDAs, zones, LGAs, user_scopes, profiles
  ...
  const [
    ...
    supabaseClient.from('mdas').select('id, name').order('name', { ascending: true }),
    supabaseClient.from('zones').select('id, name').order('name', { ascending: true }),
    supabaseClient.from('lgas').select('id, name, zone_id').order('name', { ascending: true }),
    supabaseClient.from('user_scopes').select('id, user_id, mda_id, zone_id, lga_id'),
    supabaseClient.from('profiles').select('user_id, email, full_name, global_role').order('full_name', { ascending: true })
  ]);

  if (mdasError || zonesError || lgasError || scopesError || profilesError) {
...
function renderScopesTable(scopes) {
  ...
  scopes.forEach((s) => {
    ...
    const mda = allMdas.find((m) => m.id === s.mda_id);
    const zone = s.zone_id ? allZones.find((z) => z.id === s.zone_id) : null;
    const lga = s.lga_id ? allLgas.find((l) => l.id === s.lga_id) : null;
    ...
    tdZone.className = 'px-2 py-1 align-middle';
    tdZone.textContent = zone ? zone.name : 'All zones';
    tr.appendChild(tdZone);

    const tdLga = document.createElement('td');
    tdLga.className = 'px-2 py-1 align-middle';
    tdLga.textContent = lga ? lga.name : (s.zone_id ? 'All LGAs' : 'All LGAs');
